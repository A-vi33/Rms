    ...
    const unsub = onAuthStateChanged(a, async (u) => {
      ...
      if (db) {
        try {
          ...
          if (!roleDoc.exists()) {
             // If config missing, deny access for safety
             console.error(`Role configuration for '${roleId}' not found in Firestore.`)
             await signOut(a)
             window.alert(`System Configuration Error: Role '${roleId}' is not configured in the database.`)
             navigate('/', { replace: true })
             return
          }
          ...
          const required = (allowedEmail || '').toLowerCase()
          
          if (required && uEmail !== required) {
            await signOut(a)
          ...
          console.error('Error verifying role access in Portal:', err)
          // Optionally deny access on error, but for now we might let it slide or show error
        }
      }
